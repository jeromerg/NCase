        ...
        : IGenerateCaseVisitor<ITreeNode>
    {
                                              ...
                                              [NotNull] GenerateOptions options)
        {
            ...
            if (node.TreeFact != null)
            {
                ...
                IEnumerable<List<INode>> treeFactCases = dir.Visit(node.TreeFact, options);

                if(treeFactCases == null)
                    throw new InvalidOperationException(string.Format("Visit of child {0} returned null", node.TreeFact));

                foreach (List<INode> caseOfTreeFact in treeFactCases) 
                {
                    if(caseOfTreeFact == null)
                        throw new InvalidOperationException(string.Format("Visit of child {0} returned a case having the list of facts equal to null", node.TreeFact));
                                                               ...
                                                               [NotNull] GenerateOptions options)
        {
            ...
            {
                foreach (INode branchNode in node.Branches)
                {
